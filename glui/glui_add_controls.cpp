...
GLUI_HSlider	*GLUI:: add_slider( int resizable,
					int data_type,
					void *live_var,
					int id,
					GLUI_Update_CB callback )
{
  return add_slider_to_panel( main_panel,
			      resizable, data_type, live_var, id, callback );
}


/*********************************** GLUI:: add_slider_to_panel() **********/

GLUI_HSlider	*GLUI::add_slider_to_panel( GLUI_Panel *panel,
					  int resizable,
					  int	data_type,
					  void	*live_var,
						int id,
					  GLUI_Update_CB callback )
{
  GLUI_HSlider *control;

	// MJB
	// fprintf( stderr, "add_slider: resizable = %d, callback = 0x%08x, live_type = %d, live_var = 0x%0x\n",
		// resizable, (int)callback, data_type, (int)live_var );
  
  control = new GLUI_HSlider;
  
  if ( control ) {
    control->button_is_resizable	= resizable;
    control->user_id     = id;
    control->callback    = callback;
    control->live_type = data_type;
    control->set_ptr_val( live_var );
    add_control( panel, control );
    return control;
  }
  else {
