...
void myGlutDisplay( void )
{
  ...
  if ( wireframe )
    glutWireTorus( .2,.5,segments[1],segments[0] );
  else
    glutSolidTorus( .2,.5,segments[1],segments[0] );
...
int main(int argc, char* argv[])
{
  ...
  GLUI_Spinner *segment_spinner = 
    glui->add_spinner( "Segments:", GLUI_SPINNER_INT, &segments );
		*/
	GLUI_HSlider *segment_spinner =
		glui->add_slider( true, GLUI_HSLIDER_INT, &segments );
  segment_spinner->set_int_limits( 3, 60 ); 
  //segment_spinner->set_w( 56 );
	segment_spinner->sync_slider();
